...
*   **Receding-Horizon MPC:** An independent per-module optimizer utilizing the **Adam optimizer** via JAX’s `value_and_grad` functions. It dynamically optimizes fusion input to maximize electricity output while maintaining thermal safety.
*   **Stochastic Monte-Carlo (MC) Validation:** A dedicated validation plugin for non-deterministic neutronics. It uses JAX’s PRNG keys to simulate probabilistic blanket capture and tritium breeding rates.
